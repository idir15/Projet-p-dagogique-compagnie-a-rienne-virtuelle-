...
var form_reserv=document.querySelector('.container_reservation');

    // Show date picker on click
    _self.showPicker = function() {
      form_reserv.classList.add('formresrvation__flou');
      _self.buildPicker();
    ...
    _self.hidePicker = function(event) {
      form_reserv.classList.remove('formresrvation__flou');
      setTimeout(function() {
  ...
  var Calendar = {
    ...
    buildYears: function() {
      ...
      var year = this.year();
      for (var i = year -100; i <= currentYear+20 ; i++) {
        elem += '<option value="' + i + '"';
        if (i === year) {
          elem += ' selected';
        }
