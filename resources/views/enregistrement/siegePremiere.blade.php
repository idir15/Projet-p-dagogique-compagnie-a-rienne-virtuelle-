<head>
  ...
  <link rel="stylesheet" href="/css/reservation.css" type="text/css" charset="utf-8" /></head>
{{-- {{dd($affectation)}}  --}}
...
@for($i=0; $i< count($listeEnregistre); $i++)
 ...
 @foreach ($ToutNumSiege as $key)
       ...
       @if($colone == 'A')
           <div class="airplane_seats__row airplane_seats__row--premier"> 
   ...
   @endif
         ...
         @if(in_array($key, $siegeOccupe))
              @if($colone == 'A'||$colone == 'F')
                <div class="airplane_seats__seat airplane_seats__seat_premier airplane_seats__seat_top seat--busy " id="{{$key}}">{{$colone}}</div>
              @else
                <div class="airplane_seats__seat airplane_seats__seat_premier airplane_seats__seat_bottom seat--busy " id="{{$key}}">{{$colone}}</div>
              @endif
     ...
     @else
      ...
      @if(in_array($key, $affectation))
                @if($colone == 'A'||$colone == 'F')
                  <div class="airplane_seats__seat airplane_seats__seat_premier airplane_seats__seat_top seat--selected " id="{{$key}}">{{array_search($key, $affectation)}}</div>
                @else
                  <div class=" airplane_seats__seat airplane_seats__seat_premier airplane_seats__seat_bottom seat--selected " id="{{$key}}">{{array_search($key, $affectation)}}</div>
                @endif

              @else
              @if($colone == 'A'|| $colone == 'F')
                <div class=" airplane_seats__seat airplane_seats__seat_premier airplane_seats__seat_top" id="{{$key}}">{{$colone}}</div>
              @else
                <div class="airplane_seats__seat airplane_seats__seat_premier airplane_seats__seat_bottom" id="{{$key}}">{{$colone}}</div>
              @endif
